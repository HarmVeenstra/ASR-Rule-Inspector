<#
.SYNOPSIS
This script inspects and reports on Attack Surface Reduction (ASR) rules, Controlled Folder Access (CFA) configurations, and Intune policy assignments for devices managed by Microsoft Intune.
...
4. Retrieves ASR rules, exclusions, and CFA configurations from Intune policies.
5. Compares Intune configurations with local device settings.
6. Identifies duplicates, conflicts, and mismatches in ASR rules, exclusions, and CFA configurations.
7. Evaluates Intune assignment filters and policy assignments for a specific device.
8. Generates an HTML report summarizing the findings.
...
function New-HTMLReport {
    param (
        ...
        [array]$ASRPerRuleExclusions,
        [array]$DuplicateASRPerRuleExclusions,
        [array]$CFAStatusReport,
        [array]$SameCFAReport
    )
...
"@
    ...
    if ($ASRPerRuleExclusions) {
        $html += "<h2>ASR Rule Exclusions per Rule</h2>"
        $html += "<table id='ASRPerRuleExclusions'><thead><tr><th>Policy Name</th><th>Policy Type</th><th>Friendly Name</th><th>Intune Path</th><th>Local Path</th><th>Type</th><th>Result</th><th>Note</th></tr></thead><tbody>"
        foreach ($exclusion in $ASRPerRuleExclusions) {
            $resultClass = switch ($exclusion.Result) {
                "Match" { "match" }
                "Warning" { "warning" }
                "Duplicate" { "duplicate" }
            ...
            $html += "<tr><td>$($exclusion.PolicyName)</td><td>$($exclusion.PolicyType)</td><td>$($exclusion.FriendlyName)</td><td>$($exclusion.IntunePath)</td><td>$($exclusion.LocalPath)</td><td>$($exclusion.Type)</td><td class='$resultClass'>$($exclusion.Result)</td><td>$($exclusion.Note)</td></tr>"
        }
        $html += "</tbody></table>"
    }

    if ($DuplicateASRPerRuleExclusions) {
        $html += "<h2>Duplicate/Conflicting ASR Exclusions per Rule</h2>"
        $html += "<table id='DuplicateASRPerRuleExclusions'><thead><tr><th>Policy Name</th><th>Policy Type</th><th>Friendly Name</th><th>Intune Path</th><th>Local Path</th><th>Type</th><th>Result</th><th>Note</th></tr></thead><tbody>"
        foreach ($exclusion in $DuplicateASRPerRuleExclusions) {
            $html += "<tr><td>$($exclusion.PolicyName)</td><td>$($exclusion.PolicyType)</td><td>$($exclusion.FriendlyName)</td><td>$($exclusion.IntunePath)</td><td>$($exclusion.LocalPath)</td><td>$($exclusion.Type)</td><td class='duplicate'>$($exclusion.Result)</td><td>$($exclusion.Note)</td></tr>"
        }
        $html += "</tbody></table>"
    }
    ...
    if ($CFAStatusReport) {
        ...
        foreach ($status in $CFAStatusReport) {
    ...
    if ($SameCFAReport) {
        ...
        foreach ($conflict in $SameCFAReport) {
...
Function Get-IntuneConfiguredASRRules {
    ...
    foreach ($policy in $script:AllPolicies) {
        try {
            ...
            $AssignmentResponse = Invoke-MgGraphRequest -Method GET -Uri $Assignments -ErrorAction Stop
...
function Get-ASRPerRuleStatusExclusions {
    # Dynamically generate per-rule exclusion setting definition IDs with friendly names
    $ASRPerRuleExclusions = $script:IntuneConfigurableASRRules | ForEach-Object {
        @{
            SettingDefinitionId  = "$($_.IntuneASRSettingName)_perruleexclusions"
            FriendlyName         = $_.Name
            GUID                 = $_.GUID
            IntuneASRSettingName = $_.IntuneASRSettingName
        }
    }

    $matchingPolicies = $script:AllPolicies | Where-Object {
        $_.settings.settinginstance.groupSettingCollectionValue.children | Where-Object {
            $_.choicesettingvalue.children.settingDefinitionId -in ($ASRPerRuleExclusions.SettingDefinitionId)
        }
    }

    if ($matchingPolicies.Count -eq 0) {
        Write-Host "No per-rule ASR policies found with the specified setting definition ID." -ForegroundColor Yellow
        return @()
    }

    $IntuneASRPerRuleExclusions = @()
    $global:ASRExclusionsReport = @()
    $global:SameExclusionsReport = @()
    ...
    foreach ($policy in $matchingPolicies) {
        ...
        if ($PolicyAssignments) {
            ...
            $ASRPerRuleExclusionsSettings = ($policy.settings.settinginstance.groupSettingCollectionValue.children.choicesettingvalue.children | 
                Where-Object { $_.settingDefinitionId -in ($ASRPerRuleExclusions.SettingDefinitionId) })

            foreach ($setting in $ASRPerRuleExclusionsSettings) {
                # Find the corresponding ASR rule for this specific setting
                $settingDefinitionId = $setting.settingDefinitionId
                $matchingRule = $ASRPerRuleExclusions | Where-Object { $_.SettingDefinitionId -eq $settingDefinitionId }
                
                if (!$matchingRule) {
                    continue
                }
                
                $excludedPaths = $setting.simpleSettingCollectionValue.value
                
                foreach ($ExcludedPath in $excludedPaths) {
                    if ($ExcludedPath -match "\.\w+$") {
                        $IntuneASRPerRuleExclusions += [PSCustomObject]@{ 
                            Type         = "File"
                            Path         = $ExcludedPath 
                            PolicyName   = $policy.name
                            ...
                            FriendlyName = $matchingRule.FriendlyName
                        }
                    }
                    else {
                        $IntuneASRPerRuleExclusions += [PSCustomObject]@{ 
    ...
    # Local Per Rule Exclusions
    try {
        $LocalASRPerRuleExclusions = (Get-MpPreference).AttackSurfaceReductionRules_RuleSpecificExclusions
        
        if (-not $LocalASRPerRuleExclusions -or $LocalASRPerRuleExclusions.Count -eq 0) {
            return "No ASR per rule exclusions found locally"
        }
    }
    catch {
        return "Error retrieving local ASR per rule exclusions: $_"
    }

    # Process Local Per Rule Exclusions
    $LocalASRPerRuleExclusionResults = foreach ($exclusion in $LocalASRPerRuleExclusions) {
        try {
            # Validate if the path is in a valid format before using Test-Path
            if ([System.IO.Path]::IsPathRooted($exclusion) -and 
                -not [System.String]::IsNullOrEmpty($exclusion) -and
                -not ($exclusion -match '[/\\][/\\]|[:][:]')) {
                
                if (Test-Path -LiteralPath $exclusion -ErrorAction Stop) {
                    if ($exclusion -match "\.\w+$") {
                        [PSCustomObject]@{ 
                            Type = "File"
                            Path = $exclusion 
            ...
            # Handle any exceptions during path testing
            [PSCustomObject]@{ 
                Type  = "Error"
                Path  = $exclusion 
    ...
    # Comparison and Reporting - Create separate report entries for each unique rule
    foreach ($IntuneExclusion in $IntuneASRPerRuleExclusions) {
        $LocalMatch = $LocalASRPerRuleExclusionResults | Where-Object { $_.Path -eq $IntuneExclusion.Path }
        
        $global:ASRExclusionsReport += [PSCustomObject]@{
            PolicyName   = $IntuneExclusion.PolicyName
            PolicyType   = $IntuneExclusion.PolicyType
            IntunePath   = $IntuneExclusion.Path
            LocalPath    = if ($LocalMatch) { $IntuneExclusion.Path } else { "Not Found" }
            Type         = $IntuneExclusion.Type
            FriendlyName = $IntuneExclusion.FriendlyName
            Result       = if ($LocalMatch) { "Match" } else { "No Match" }
            Note         = if (-not $LocalMatch) { "No matching local exclusion found" } else { "" }
        }
    }

    # Check for duplicates - but handle differently now that we're processing at the individual rule level
    $duplicateGroups = $global:ASRExclusionsReport | Group-Object -Property IntunePath | Where-Object { $_.Count -gt 1 }
    ...
    foreach ($group in $duplicateGroups) {
        ...
        foreach ($policyNameGroup in $uniquePolicyNameGroups) {
            $global:SameExclusionsReport += [PSCustomObject]@{
                ...
                LocalPath    = $policyNameGroup.Group[0].LocalPath
                Type         = $policyNameGroup.Group[0].Type
                FriendlyName = $policyNameGroup.Group[0].FriendlyName
                Result       = $result
                Note         = $policyNameGroup.Group[0].Note
            }
        }
...
function Test-DevicePolicyAssignment {
    ...
    if ($deviceProperties.AzureAdDeviceId) {
        ...
        try {
            ...
            $azureAdDevice = Get-MgDevice -Filter "DeviceId eq '$($deviceProperties.AzureAdDeviceId)'" -ErrorAction Stop
                    
            if ($azureAdDevice) {
                # Get device group memberships
                $deviceGroups = Get-MgDeviceMemberOf -DeviceId $azureAdDevice.Id -ErrorAction Stop
...
try {
    ...
    # Filter ASR data to only include rules from applicable policies
    $applicablePolicyNames = $applicablePolicies.name
    $filteredASRData = $ASRData | Where-Object { $applicablePolicyNames -contains $_.Policy }
    
    # Get duplicates but only for applicable policies
    $DuplicateASRRules = Find-DuplicateASRRules | Where-Object { $applicablePolicyNames -contains $_.Policy }
    
    # Get exclusions but filter for applicable policies
    $allASRExclusions = Get-ASRStatusExclusions
    $ASRExclusions = $allASRExclusions.ASRExclusionsReport | Where-Object { $applicablePolicyNames -contains $_.Policy }
    $DuplicateASRExclusions = $allASRExclusions.SameExclusionsReport | Where-Object { $applicablePolicyNames -contains $_.Policy }
    
    # Get per-rule exclusions but filter for applicable policies
    $ASRPerRuleExclusionsData = Get-ASRPerRuleStatusExclusions
    $ASRPerRuleExclusions = $ASRPerRuleExclusionsData.ASRExclusionsReport | Where-Object { $applicablePolicyNames -contains $_.PolicyName }
    $DuplicateASRPerRuleExclusions = $ASRPerRuleExclusionsData.SameExclusionsReport | Where-Object { $applicablePolicyNames -contains $_.PolicyName }
    
    # Get CFA status but filter for applicable policies
    $CFAStatus = Get-CFAStatus
    $CFAStatusReport = $CFAStatus.CFAStatusReport | Where-Object { $applicablePolicyNames -contains $_.PolicyName }
    $SameCFAReport = $CFAStatus.SameCFAReport | Where-Object { $applicablePolicyNames -contains $_.PolicyName }
    
    
    Write-Host "Applicable policies: $($applicablePolicyNames -join ', ')" -ForegroundColor Cyan
    Write-Host "Filtered ASR rules: $($filteredASRData.Count)" -ForegroundColor Yellow
    ...
    Write-Host "Generating ASR per rule exclusions report..." -ForegroundColor Cyan
...
# Update function call to include DuplicateASRPerRuleExclusions parameter
New-HTMLReport -ASRRules $filteredASRData -DuplicateASRRules $DuplicateASRRules -ASRExclusions $ASRExclusions -DuplicateASRExclusions $DuplicateASRExclusions -CFAStatusReport $CFAStatusReport -SameCFAReport $SameCFAReport -ASRPerRuleExclusions $ASRPerRuleExclusions -DuplicateASRPerRuleExclusions $DuplicateASRPerRuleExclusions
